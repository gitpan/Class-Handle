...
# Provides a friendly object interface to Classes.
...
sub installed {
	...
	# Pass through to Class::Inspector
	return Class::Inspector->installed( $self->{name} );
}
...
sub loaded {
	...
	# Pass through to Class::Inspector
	return Class::Inspector->loaded( $self->{name} );
}
...
sub filename {
	...
	# Pass through to Class::Inspector
	return Class::Inspector->filename( $self->{name} );
}
...
sub resolved_filename {
	...
	# Pass through to Class::Inspector
	return Class::Inspector->resolved_filename( $self->{name} );
}
...
sub loaded_filename {
	...
	# Pass through to Class::Inspector
	return Class::Inspector->loaded_filename( $self->{name} );
}
...
sub functions {
	...
	return Class::Inspector->functions( $self->{name} );
}
...
sub function_refs {
	...
	# Pass through to Class::Inspector.
	# ( It will check that the class is loaded )
	return Class::Inspector->function_refs( $self->{name} );
}
...
sub function_exists {
	...
	# Pass through to Class::Inspector.
	# ( It will check that the class is loaded )
	return Class::Inspector->function_exists( $self->{name}, $function );
}
...
sub load {
	...
	my $import = $self->can( 'import' );
	return 1 unless $import;
...
C<Class::ISA->super_path> method. Returns an ordered list of
